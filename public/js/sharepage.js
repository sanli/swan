...
$.extend($M, {
  _post : function(url, args, options){
    ...
    $.post(url, { payload : JSON.stringify(args)}, function(data){
      if(data.R === 'Y'){
        options.successfn && options.successfn(data.M);
      }else{
        $.alert( alertPosition , options.errmsg + data.M, 30000);
        options.failfn && options.failfn(data.M || '发生错误');
      } 
    }, 'json');
  },
  ...
  fillResult : function(target, options){
    ...
    var outhtml = $.template(tempId, options);
  ...
  // 展开页面上的sharepage宏代码
  expandSharepageMacro : function(){
    $('[data-sharepage-macro]').each(function( i , element){
        var $el = $(element),
            optTxt = $el.text(),
            tpl = $el.data('sharepage-macro');
        if(optTxt){
            var optfn = new Function('return (' + optTxt + ')');
            options = optfn();    
        }else{
            options = {};
        }
        ...
        try{
          var expand = $.template(tpl, options);
          $el.html(expand);
        }catch(e){
          console.log("展开sharepage宏代码出错, %s", $el[0] + $el.html());
          console.error(e);
          throw e;
        }
    });
  ...
  // 载入页面上定义的通用sharepage控件，这个函数会在页面载入时被自动调用一次
  // 初始化当时能够找到的所有sharepage control对象
  // 目前支持:  datetime
  createSharepageControl : function(){
    $('[data-sharepage-control]').each(function(i, el){
      var $el = $(el),
        ctltype = $el.data('sharepage-control') ;
       
      // 日期组件，http://www.malot.fr/bootstrap-datetimepicker
      if(ctltype === 'date' || ctltype === 'time' || ctltype === 'datetime'){
        $el.datetimepicker({ language : 'zh-CN', 
            weekStart: 1,
            todayBtn: true,
            autoclose: true,
            startView: ctltype === 'time' ? 1 : 2, 
            minView: (ctltype === 'time' || ctltype === 'datetime') ? 0 : 2,
            forceParse: true
        });        
      }
    });
...
(function ($) {
  ...
  $.extend({
    ...
    ask : function(title, message, ok){
      ...
      $('#confirmDlg').modal().one('shown.bs.modal', function(){
    ...
    message : function(title, message, closetime){
      $('#messageDlg').modal().one('shown.bs.modal', function(){
    ...
    showmodal : function(dlgSelector, okfn, canclefn, opt){
        ...
        return dlg.modal(opt).one('shown.bs.modal', function(){
            if(opt.title)  
              dlg.find('.modal-header  h3').text(opt.title);

            var $tgt = dlg.find("button.btn-primary");
            $tgt.button('reset').attr('disabled', false);
            $tgt.off('click').on('click', function(event){
              var $tgt = $(event.target);
              $tgt.button('doing').attr('disabled', true);
              okfn(function(err){
                $tgt.button('reset').attr('disabled', false);
                console.log(err);
                if(!err)
     ...
     * opt : state, error , info
     */
    alert : function(selector, message, closetime, opt){
      opt = opt || { state : 'error'};
      closetime = closetime || 300000;
      // var state = opt.state === 'error'  ? 'alert-danger' : 'alert-info';
      // $('.alert').alert('close');
      // $('.alert').remove();
      // $(selector).before('<div class="alert ' + state + ' fade in"><button type="button" class="close" data-dismiss="alert">×</button>'+ message +'</div>');
      // if(closetime)
      //   setTimeout(function(){
      //     $('.alert').alert('close');
      //   },closetime);
      Messenger().post({ message : message, showCloseButton: true, hideAfter : closetime / 10000 });
    },

    errorAlerter : function(){
      return function(err){
        $.alert('#cellDiv', err.message);
      }
    },
    ...
    template : function(tplId, options){
      var temp = $('#'+tplId).html();
      var fn = _.template(temp, {
              evaluate    : /<@([\s\S]+?)@>/g,
              interpolate : /<@=([\s\S]+?)@>/g,
              escape      : /<@-([\s\S]+?)@>/g
          });
      var expand = fn(options);
      return expand;
    }
  });
})(window.jQuery);
...
(function($){
  $.fn.extend({
     ...
     * checkboxAsBoolean :
     *   checkbox的值提取为一个bool值, 选中为真，没选中为false
     * checkboxSetter :
     *   为checkbox赋值时的转换工具类
     */
      ...
      return this.each(function() {
        $.each( data, function(k, v) {
          ...
          if ( settings.findbyname ) { // by name
            ...
            // 过滤查询表达式
            if(settings.queryexp && elt.data('queryexp')){
              var values = /(.*)\((.*)\)/.exec(v);
              if(values)
                v = values[2];
            }
              ...
              if(elt.is('span')){
                ...
                if(elt.attr("type") == "checkbox"){
                  if(options.checkboxAsBoolean){
                    elt.prop('checked', options.checkboxSetter ? options.checkboxSetter(v) : v); 
                  }else{
                    elt.val( [v] );
                  }
                }else{
      ...
      elts.each(function(i){
        ...
        if(input.attr('type') === 'checkbox'){
          if(!settings.checkboxAsBoolean){
            ...
            var checkv = input.prop('checked');
            data[name] = settings.checkboxGetter ? settings.checkboxGetter(checkv) : checkv  ;
          }
        ...
        // 查询表达式样
        if(settings.queryexp && input.data('queryexp')){
          var exp = input.data('queryexp');
          data[name] = exp + '(' + data[name] + ')';
        }
      });
      return data;
    },
    ...
    /**
     * 放开全部只读字段，并标记改字段，随后可以用enablereadonly()方法重新开放
     */
    disablereadonly: function(options){
      this.find('[data-sharepage-readonly]').removeProp('readonly');
      return this;
    },
     ...
     * 关闭全部只读字段
     */
    enablereadonly: function(options){
      this.find('[data-sharepage-readonly]').prop('readonly', 'true');
      return this;
    },
