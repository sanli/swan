//#!本文件由share.js自动产生, 命令行为: node share.js gen member_charge CRUD ..
'use strict';
/**
 * member_charge数据库访问类
 */
...
var member_charge = new Schema({
    ...
    // 最后修改时间
    utime : Date,
},  { collection: 'member_charge' });

// 创建索引，以及设置唯一键
// TODO: 根据实际结构确定索引结构
member_charge.index({_id : 1})
    .index({module_name : 1}, { unique : true })
    .index({module_id : 1}, { unique : true });
var member_charge = mongoose.model('member_charge', member_charge ),
    _Module = member_charge ;
exports.member_charge  = member_charge;
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('member_chargedb.js '+ testcmd.join('|'));
  }
}
