//#!本文件由share.js自动产生, 命令行为: node share.js gen member_log CRUD ..
'use strict';
/**
 * member_log数据库访问类
 */
...
var member_log = new Schema({
    ...
    // 最后修改时间
    utime : Date,
},  { collection: 'member_log' });

// 创建索引，以及设置唯一键
// TODO: 根据实际结构确定索引结构
member_log.index({_id : 1})
    .index({module_name : 1}, { unique : true })
    .index({module_id : 1}, { unique : true });
var member_log = mongoose.model('member_log', member_log ),
    _Module = member_log ;
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('member_logdb.js '+ testcmd.join('|'));
  }
}
