//#!本文件由share.js自动产生, 命令行为: node sharecli.js gen member_log
...
var Module = $.extend(new $M(), {
    ...
    createModule: function(){
        function _createModule(condition, fn, fail){
            $M.doupdate('/memberlog/create', condition, { successfn : fn , failfn: fail});
        };
    ...
    updateModuleInfo : function(condition, fn, fail){
        $M.doupdate('/memberlog/update', condition, { successfn : fn , failfn: fail});
    },

    // 查询详细信息
    loadDataDetail : function(id, fn){
        $M.doquery('/memberlog/get', {_id : id}
            , { successfn : fn , alertPosition : '#cellDiv'});
    }
