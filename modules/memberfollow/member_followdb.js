//#!本文件由share.js自动产生, 命令行为: node share.js gen member_follow CRUD ..
'use strict';
/**
 * member_follow数据库访问类
 */
...
var member_follow = new Schema({
    ...
    // 最后修改时间
    utime : Date,
},  { collection: 'member_follow' });

// 创建索引，以及设置唯一键
// TODO: 根据实际结构确定索引结构
member_follow.index({_id : 1})
    .index({module_name : 1}, { unique : true })
    .index({module_id : 1}, { unique : true });
var member_follow = mongoose.model('member_follow', member_follow ),
    _Module = member_follow ;
exports.member_follow  = member_follow;
...
if(isme(__filename)){
  if(process.argv.length > 2 && isme(__filename)){
    ...
    console.log('member_followdb.js '+ testcmd.join('|'));
  }
}
